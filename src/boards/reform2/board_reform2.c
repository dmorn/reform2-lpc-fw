...
#define REFORM_MBREV_30_R1 30 // motherboard 3.0
...
// deep sleep is disabled by default as it doesn't save much power and causes long wakeup times
//#define REFORM_DEEP_SLEEP
...
#elif REFORM_MOTHERBOARD_REV == 30
#  define FW_STRING2 "30_R1"
#endif
#ifndef FW_STRING3
#  define FW_STRING3 "20250701"
#endif
...
#if REFORM_MOTHERBOARD_REV >= REFORM_MBREV_30_R1
#  define CHARGE_SOFT_START 0
#else
#  define CHARGE_SOFT_START 500
#endif
...
// 2.2A x 3600 seconds/hour
#define MAX_CAPACITY (2.2)*3600.0
...
#define MISSING_VALUE_LO 0.4
  ...
  #define FULLY_CHARGED_CURRENT -0.1
#else
...
void measure_cell_voltages_and_control_discharge() {
  ...
  if (state != ST_CHARGE && discharge_bits != 0) {
    // we're discharging (balancing),
    // so disable charge current
...
void measure_and_accumulate_current() {
  ...
  // enable charge current by default,
  // can be turned off for balancing in
...
int soft_start = CHARGE_SOFT_START;
int charge_ramp = 0;
void measure_and_accumulate_current_v25() {
  ...
  if (state == ST_CHARGE && discharge_bits == 0) {
    if (soft_start > 0) {
      // still in soft start phase, no charging yet
      disable_charge_current();
      soft_start--;
    } else {
      // soft start complete, enable charging
      enable_charge_current();
    }
    delay(10);
    volts   = (float)i2c_read16_le(INA233_ADDRESS, 0x88);
    current = (float)i2c_read16_le(INA233_ADDRESS, 0x89);

    if (soft_start <= 0) {
      // soft start phase complete, start ramp phase
      charge_ramp++;
      if (charge_ramp > 100 && current >= 0) {
        // if charging has stopped, reset soft_start
        soft_start = CHARGE_SOFT_START;
        charge_ramp = 0;
      }
    }
  }
  ...
  volts   = (float)i2c_read16_le(INA233_ADDRESS, 0x88);
  current = (float)i2c_read16_le(INA233_ADDRESS, 0x89);
  volts   *= 0.00125; // TODO adjust for 20mOhms
  current *= 0.00030517578125;
...
void handle_spi_commands() {
  ...
  if (spi_debug_enabled || spi_buf[0] != 0xb5) {
    sprintf(uartBuffer, "[spi rx %d] ", spi_rxlen);
    uartSend((uint8_t*)uartBuffer, strlen(uartBuffer));
    for (int i = 0; i < spi_rxlen; i++) {
      sprintf(uartBuffer, "%2x ", spi_buf[i]);
      uartSend((uint8_t*)uartBuffer, strlen(uartBuffer));
    }
    sprintf(uartBuffer, "\t");
    uartSend((uint8_t*)uartBuffer, strlen(uartBuffer));
    for (int i = 0; i < spi_rxlen; i++) {
      if (spi_buf[i] >= 32) {
        sprintf(uartBuffer, "%c", spi_buf[i]);
      } else {
        sprintf(uartBuffer, ".");
      }
      uartSend((uint8_t*)uartBuffer, strlen(uartBuffer));
  ...
  if (spi_command == 'p') {
    ...
    if (spi_arg1 == 0) memcpy(spi_buf, FW_STRING1, 8);
    else if (spi_arg1 == 1) memcpy(spi_buf, FW_STRING2, 8);
    else memcpy(spi_buf, FW_STRING3, 8);
  }
  ...
  for (uint8_t i = 0; i < 8; i++) {
    ...
    while ((LPC_SSP0->SR & SSP0_SR_RNE_MASK) == SSP0_SR_RNE_EMPTY) {
      ...
      timeout--;
      if (timeout <= 0) {
        sprintf(uartBuffer, "[rx drain timeout @%d]\r\n", i);
        uartSend((uint8_t*)uartBuffer, strlen(uartBuffer));
        break;
      }
    }
  ...
  if (spi_command == 'p') {
    ...
    // pass message byte directly to uart (implemented for Desktop Reform control panel)
    uartSend(&spi_arg1, 1);
  }
}
...
int main(void) {
  ...
  while (1) {
    ...
    if (REFORM_MOTHERBOARD_REV >= REFORM_MBREV_25_R2) {
      measure_and_accumulate_current_v25(); // 10ms delay
    } else {
